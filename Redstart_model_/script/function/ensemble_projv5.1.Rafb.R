...
# v5.1 -> include bin and prob ENSEMBLE
                               ...
                               outpath=outpath)
{
  ...
  if (gbm.mod==TRUE)
  {
    ...
    for (i in 1:n.rep.gbm)
    {
      ...
      if (i==1 && n.rep.gbm==1)
      {
        ...
        assign(paste("gbm.prob.proj.",i,sep=""),
               raster(paste(inpath,"proj_RaFB_gbm_prob_rep_",i,"_",scenario,".tif",sep=""))) 
      }
      
      if (i ==1)
      {
        sum.gbm.bin.proj <- get(paste("gbm.bin.proj.",i,sep=""))
        sum.gbm.prob.proj <- get(paste("gbm.prob.proj.",i,sep=""))
      } else {
        sum.gbm.bin.proj <- sum.gbm.bin.proj + get(paste("gbm.bin.proj.",i,sep=""))
        sum.gbm.prob.proj <- sum.gbm.prob.proj + get(paste("gbm.prob.proj.",i,sep=""))
      }
    ...
    ensemble.prob.all <- sum.gbm.prob.proj/n.rep.gbm
    # End of loop GBM					
  }
  ###################################################################################################################################################
  # ME
  if (me.mod==TRUE)
  {
    ...
    for (i in 1:n.rep.me)
    {
      ...
      if (i==1 && n.rep.gbm==1)
      {
        ...
        assign(paste("me.prob.proj.",i,sep=""),
               raster(paste(inpath,"proj_RaFB_me_prob_rep_",i,"_",scenario,".tif",sep=""))) 
      }
      
      if (i ==1)
      {
        sum.me.bin.proj <- get(paste("me.bin.proj.",i,sep=""))
        sum.me.prob.proj <- get(paste("me.prob.proj.",i,sep=""))
      } else {
        sum.me.bin.proj <- sum.me.bin.proj + get(paste("me.bin.proj.",i,sep=""))
        sum.me.prob.proj <- sum.me.prob.proj + get(paste("me.prob.proj.",i,sep=""))
      }
    ...
    if (gbm.mod==FALSE)
    {
      ...
      ensemble.prob.all <- sum.me.prob.proj/n.rep.me
      
    } else	
    {
      ensemble.bin.all <- ensemble.bin.all + sum.me.bin.proj
      ensemble.prob.all <- (ensemble.prob.all + sum.me.prob.proj/n.rep.me)/2
    }
    # End of loop ME			
  }
  ###################################################################################################################################################
  ...
  if (rf.mod==TRUE)
  {
    ...
    for (i in 1:n.rep.rf)
    {
      ...
      if (i==1 && n.rep.gbm==1)
      {
        ...
        assign(paste("rf.prob.proj.",i,sep=""),
               raster(paste(inpath,"proj_RaFB_rf_prob_rep_",i,"_",scenario,".tif",sep=""))) 
      }
      
      if (i ==1)
      {
        sum.rf.bin.proj <- get(paste("rf.bin.proj.",i,sep=""))
        sum.rf.prob.proj <- get(paste("rf.prob.proj.",i,sep=""))
      } else {
        sum.rf.bin.proj <- sum.rf.bin.proj + get(paste("rf.bin.proj.",i,sep=""))
        sum.rf.prob.proj <- sum.rf.prob.proj + get(paste("rf.prob.proj.",i,sep=""))
      }
    ...
    if (gbm.mod==FALSE && me.mod==FALSE)
    {
      ...
      ensemble.prob.all <-  sum.rf.prob.proj/n.rep.rf
      
    } else	
    {
      ensemble.bin.all <- ensemble.bin.all + sum.rf.bin.proj
      ensemble.prob.all <- (ensemble.prob.all + sum.rf.prob.proj/n.rep.rf)/2
    }
  ...
  if (glm.modav.mod==TRUE)
  {
    ...
    for (i in 1:n.rep.glm.modav)
    {
      ...
      if (i==1 && n.rep.glm.modav==1)
      {
        ...
        assign(paste("glm.modav.prob.proj.",i,sep=""),
               raster(paste(inpath,"proj_RaFB_glm_prob_modav_",scenario,".tif",sep=""))) 
      } else {
        
        assign(paste("glm.modav.bin.proj.",i,sep=""),
               raster(paste(inpath,"proj_RaFB_glm_bin_modav_rep_",i,"_",scenario,".tif",sep=""))) 
        assign(paste("glm.modav.prob.proj.",i,sep=""),
               raster(paste(inpath,"proj_RaFB_glm_prob_modav_rep_",i,"_",scenario,".tif",sep="")))
      }
      
      if (i ==1)
      {
        sum.glm.modav.bin.proj <- get(paste("glm.modav.bin.proj.",i,sep=""))
        sum.glm.modav.prob.proj <- get(paste("glm.modav.prob.proj.",i,sep=""))
      } else {
        sum.glm.modav.bin.proj <- sum.glm.modav.bin.proj + get(paste("glm.modav.bin.proj.",i,sep=""))
        sum.glm.modav.prob.proj <- sum.glm.modav.prob.proj + get(paste("glm.modav.prob.proj.",i,sep=""))
      }
    ...
    if (gbm.mod==FALSE && me.mod==FALSE && rf.mod==FALSE)
    {
      ...
      ensemble.prob.all <-  sum.glm.modav.prob.proj/n.rep.glm.modav
      
    } else	
    {
      ensemble.bin.all <- ensemble.bin.all + sum.glm.modav.bin.proj
      ensemble.prob.all <- (ensemble.prob.all + sum.glm.modav.prob.proj/n.rep.glm.modav)/2
    }
  ...
  if (glm.stepwise.mod==TRUE)
  {
    ...
    for (i in 1:n.rep.glm.stepwise)
    {
      ...
      if (i==1 && n.rep.glm.stepwise==1)
      {
        ...
        assign(paste("glm.stepwise.prob.proj.",i,sep=""),
               raster(paste(inpath,"proj_RaFB_glm_prob_stepwise_",scenario,".tif",sep=""))) 
      } else {
        assign(paste("glm.stepwise.bin.proj.",i,sep=""),
               raster(paste(inpath,"proj_RaFB_glm_bin_stepwise_rep_",i,"_",scenario,".tif",sep=""))) 
        assign(paste("glm.stepwise.prob.proj.",i,sep=""),
               raster(paste(inpath,"proj_RaFB_glm_prob_stepwise_rep_",i,"_",scenario,".tif",sep=""))) 
      ...
      if (i ==1)
      {
        ...
        sum.glm.stepwise.prob.proj <- get(paste("glm.stepwise.prob.proj.",i,sep=""))
      } else {
        sum.glm.stepwise.bin.proj <- sum.glm.stepwise.bin.proj + get(paste("glm.stepwise.bin.proj.",i,sep=""))
        sum.glm.stepwise.prob.proj <- sum.glm.stepwise.prob.proj + get(paste("glm.stepwise.prob.proj.",i,sep=""))
      }
    ...
    if (gbm.mod==FALSE && me.mod==FALSE && rf.mod==FALSE && glm.modav.mod==TRUE)
    {
      ...
      ensemble.prob.all <-  sum.glm.stepwise.prob.proj/n.rep.glm.stepwise
      
    } else	
    {
      ensemble.bin.all <- ensemble.bin.all + sum.glm.stepwise.bin.proj
      ensemble.prob.all <- (ensemble.prob.all + sum.glm.stepwise.prob.proj/n.rep.glm.stepwise)/2
    }
  ...
  if (gam.stepwise.mod==TRUE)
  {
    ...
    for (i in 1:n.rep.gam.stepwise)
    {
      ...
      if (i==1 && n.rep.gam.stepwise==1)
      {
        ...
        assign(paste("gam.stepwise.prob.proj.",i,sep=""),
               raster(paste(inpath,"proj_RaFB_gam_prob_stepwise_",scenario,".tif",sep=""))) 
        ...
        assign(paste("gam.stepwise.prob.proj.",i,sep=""),
               raster(paste(inpath,"proj_RaFB_gam_prob_stepwise_rep_",i,"_",scenario,".tif",sep=""))) 
      ...
      if (i ==1)
      {
        ...
        sum.gam.stepwise.prob.proj <- get(paste("gam.stepwise.prob.proj.",i,sep=""))
      } else {
        sum.gam.stepwise.bin.proj <- sum.gam.stepwise.bin.proj + get(paste("gam.stepwise.bin.proj.",i,sep=""))
        sum.gam.stepwise.prob.proj <- sum.gam.stepwise.prob.proj + get(paste("gam.stepwise.prob.proj.",i,sep=""))
    ...
    if (gbm.mod==FALSE && me.mod==FALSE && rf.mod==FALSE && glm.modav.mod==TRUE && glm.stepwise.mod==FALSE)
    {
      ...
      ensemble.prob.all <-  sum.gam.stepwise.prob.proj/n.rep.gam.stepwise
      
    } else	
    {
      ensemble.bin.all <- ensemble.bin.all + sum.gam.stepwise.bin.proj
      ensemble.prob.all <- (ensemble.prob.all + sum.gam.stepwise.prob.proj/n.rep.gam.stepwise)/2
    }
  ...
  writeRaster(ensemble.prob.all,paste(outpath,"ENSEMBLE_prob_",species,"_",ens.code,"_",scenario,".tif",sep=""),
              datatype="FLT8S", overwrite=TRUE)
